document.querySelector('#login').addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(staff.ok){
      ...
      const destination=result.role==='sayman'?'sayman.html':result.role==='moderator'?'moderator.html':result.role==='author'?'yazar-paneli.html':'yonetim.html';
    ...
    location.href='uye-paneli.html';
  }catch(x){
    error.textContent=x.message;
  }
});
document.querySelector('#login button').insertAdjacentHTML('afterend','<a href="sifremi-unuttum.html" style="display:block;text-align:right;margin-top:12px;font-size:12px">Şifremi unuttum</a>');

...
function render(){
  ...
  grid.innerHTML=shown.map((x,i)=>`<a class="editorial-card" style="--i:${i}" data-id="${esc(x.id)}" href="icerik-detay.html?id=${encodeURIComponent(x.id)}">
  ...
  grid.querySelectorAll('.editorial-card').forEach(card=>card.addEventListener('click',e=>{
    ...
    e.preventDefault();
    openDetail(articles.find(x=>x.id===card.dataset.id));
  }));
}

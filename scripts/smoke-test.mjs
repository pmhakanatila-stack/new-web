...
try{
  ...
  const persisted=JSON.parse(await readFile(join(temp,'cms.json'),'utf8'));if(!persisted.content.some(x=>x.title===title))throw new Error('Sunucu yeniden başlayınca veri kayboldu');const remainingLegacySources=persisted.content.filter(x=>/^https?:\/\/(?:www\.)?peyzajder\.org/i.test(String(x.sourceUrl||''))).map(x=>`${x.id}: ${x.sourceUrl}`);if(remainingLegacySources.length)throw new Error(`Eski kaynak bağlantısı kalıcı veriden temizlenmedi: ${remainingLegacySources.join(', ')}`);

...
const LOGIN_MAX_ATTEMPTS=6,LOGIN_WINDOW_MS=10*60*1000,LOGIN_LOCK_MS=15*60*1000;
const clientKey=req=>String(req.headers['x-forwarded-for']||'').split(',')[0].trim()||req.socket?.remoteAddress||'unknown';
const loginBlocked=key=>{const rec=loginAttempts.get(key);if(!rec)return 0;if(rec.lockedUntil&&rec.lockedUntil>Date.now())return rec.lockedUntil-Date.now();if(rec.lockedUntil&&rec.lockedUntil<=Date.now())loginAttempts.delete(key);return 0};
const recordLoginFailure=key=>{const now=Date.now(),rec=loginAttempts.get(key)||{count:0,firstAt:now};if(now-rec.firstAt>LOGIN_WINDOW_MS){rec.count=0;rec.firstAt=now}rec.count++;if(rec.count>=LOGIN_MAX_ATTEMPTS)rec.lockedUntil=now+LOGIN_LOCK_MS;loginAttempts.set(key,rec)};
const recordLoginSuccess=key=>loginAttempts.delete(key);
const SECURITY_HEADERS={'X-Content-Type-Options':'nosniff','X-Frame-Options':'SAMEORIGIN','Referrer-Policy':'strict-origin-when-cross-origin','Permissions-Policy':'geolocation=(), microphone=(), camera=()','Strict-Transport-Security':'max-age=15552000','Content-Security-Policy':"default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'self'"};
...
const adminSession=req=>{const t1=cookies(req).peyzajder_admin_session,t2=cookies(req).peyzajder_session;return validSession(t1,sessions.get(t1))||((sessions.get(t2)?.type==='admin')?validSession(t2,sessions.get(t2)):null)};
const memberSession=req=>{const t1=cookies(req).peyzajder_member_session,t2=cookies(req).peyzajder_session;return validSession(t1,sessions.get(t1))||((sessions.get(t2)?.type==='member')?validSession(t2,sessions.get(t2)):null)};
...
function ensureDefaultStaff(){
  ...
  for(const [email,name,role] of defaults){
    if(!db.users.some(u=>String(u.email||u.username||'').toLowerCase()===email)){
      ...
      db.users.push({id:`users-${Date.now()}-${randomBytes(3).toString('hex')}`,name,email,role,status:'Aktif',systemRole:normalizedRole(role),passwordHash:scryptSync(plain,salt,32).toString('hex'),salt,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
      console.log(`PEYZAJDER: varsayılan personel oluşturuldu ${email} / geçici şifre: ${plain} (bu şifre bir daha gösterilmeyecek, not edin)`);
      changed=true;
    }
  }
  return changed;
...
const isCorporateMember=(account,application)=>{const typeText=normText(account.memberType||account.membershipType||account.group||application?.membershipType||'');if(typeText.includes('kurumsal'))return true;if(typeText.includes('bireysel')||typeText.includes('öğrenci')||typeText.includes('ogrenci'))return false;return !!(application?.organization)};
...
async function api(req,res,url){
  ...
  if(url.pathname==='/api/public/home'&&req.method==='GET'){
    ...
    const publicItem=x=>({id:x.id,title:cleanText(x.title||x.name||''),category:x.category||'',summary:cleanText(x.summary||x.description||''),body:cleanText(x.body||''),image:x.image||x.cover||'',date:x.date||x.createdAt||'',url:x.id?detailUrl(x):(x.sourceUrl||'arsiv.html'),author:x.author||''});
  ...
  if(url.pathname==='/api/public/archive'&&req.method==='GET'){
    ...
    const toArchive=(x,type,category=x.category||'haberler')=>({
      ...
      category,
      url:`icerik-detay.html?id=${encodeURIComponent(x.id||'')}`
    });
    const items=[
  ...
  if(url.pathname==='/api/register'&&req.method==='POST'){
    ...
    await save(db);
    return json(res,201,{ok:true,membershipStatus:account.membershipStatus,redirect:'uye-paneli.html?created=1'},{'Set-Cookie':`peyzajder_member_session=${token}; HttpOnly; SameSite=Strict; Path=${COOKIE_PATH}; Max-Age=28800`});
  }
  if(url.pathname==='/api/member/login'&&req.method==='POST'){const b=await body(req),email=String(b.email||'').trim().toLowerCase(),rateKey=`member:${clientKey(req)}:${email}`;const lockedMs=loginBlocked(rateKey);if(lockedMs>0)return json(res,429,{error:`Çok fazla başarısız giriş denemesi. Lütfen ${Math.ceil(lockedMs/60000)} dakika sonra tekrar deneyin.`});const account=db.accounts.find(x=>x.email===email);if(!account){recordLoginFailure(rateKey);return json(res,401,{error:'E-posta veya şifre hatalı'})}const candidate=scryptSync(String(b.password||''),account.salt,32),stored=Buffer.from(account.passwordHash,'hex');if(stored.length!==candidate.length||!timingSafeEqual(stored,candidate)){recordLoginFailure(rateKey);return json(res,401,{error:'E-posta veya şifre hatalı'})}recordLoginSuccess(rateKey);const token=randomBytes(32).toString('hex');sessions.set(token,{type:'member',accountId:account.id,user:account.email,created:Date.now()});return json(res,200,{ok:true},{'Set-Cookie':`peyzajder_member_session=${token}; HttpOnly; SameSite=Strict; Path=${COOKIE_PATH}; Max-Age=28800`})}
  if(url.pathname==='/api/member/forgot'&&req.method==='POST'){const b=await body(req),email=String(b.email||'').trim().toLowerCase(),account=db.accounts.find(x=>x.email===email);if(!account)return json(res,200,{ok:true,message:'Hesap mevcutsa sıfırlama bağlantısı oluşturuldu'});const token=randomBytes(32).toString('hex');resetTokens.set(token,{accountId:account.id,expires:Date.now()+30*60*1000});return json(res,200,{ok:true,message:'Şifre sıfırlama bağlantınız hazır',resetUrl:`sifre-sifirla.html?token=${token}`})}
  ...
  if(url.pathname==='/api/member/password'&&req.method==='PUT'){
    ...
    if(!a.passwordHash||!a.salt)return json(res,400,{error:'Hesabınız için şifre değişikliği şu anda kullanılamıyor. Lütfen yönetimle iletişime geçin'});
    const candidate=scryptSync(current,a.salt,32),stored=Buffer.from(a.passwordHash,'hex');
    if(stored.length!==candidate.length||!timingSafeEqual(stored,candidate))return json(res,401,{error:'Mevcut şifreniz hatalı'});
    if(next.length<8)return json(res,400,{error:'Yeni şifre en az 8 karakter olmalıdır'});
    const salt=randomBytes(16).toString('hex');
    a.salt=salt;a.passwordHash=scryptSync(next,salt,32).toString('hex');a.updatedAt=new Date().toISOString();
    await save(db);
    return json(res,200,{ok:true});
  }
  ...
  if(url.pathname==='/api/login'&&req.method==='POST'){const b=await body(req),login=String(b.username||'').trim().toLowerCase(),rateKey=`admin:${clientKey(req)}:${login}`;const lockedMs=loginBlocked(rateKey);if(lockedMs>0)return json(res,429,{error:`Çok fazla başarısız giriş denemesi. Lütfen ${Math.ceil(lockedMs/60000)} dakika sonra tekrar deneyin.`});let user,role;if(login===USER&&secureEqual(b.password||'',PASSWORD)){user=USER;role='admin'}else{const staff=db.users.find(x=>String(x.email||x.username||'').toLowerCase()===login&&x.status!=='Pasif');if(staff?.passwordHash){const candidate=scryptSync(String(b.password||''),staff.salt,32),stored=Buffer.from(staff?.passwordHash,'hex');if(stored.length===candidate.length&&timingSafeEqual(stored,candidate)){user=staff.email||staff.username;role=staff.systemRole||normalizedRole(staff.role)}}}if(user){recordLoginSuccess(rateKey);const token=randomBytes(32).toString('hex');sessions.set(token,{type:'admin',role,user,created:Date.now()});return json(res,200,{ok:true,user,role},{'Set-Cookie':`peyzajder_admin_session=${token}; HttpOnly; SameSite=Strict; Path=${COOKIE_PATH}; Max-Age=28800`})}recordLoginFailure(rateKey);return json(res,401,{error:'Kullanıcı adı veya şifre hatalı'})}
  ...
  if(url.pathname==='/api/author/me'){const s=adminSession(req);if(!s||s.role!=='author')return json(res,401,{error:'Köşe yazarı girişi gerekli'});const u=db.users.find(x=>String(x.email||x.username||'').toLowerCase()===String(s.user||'').toLowerCase()),email=String(u?.email||s.user||'').toLowerCase(),name=u?.name||s.user,a=(db.authors||[]).find(x=>String(x.email||'').toLowerCase()===email);return json(res,200,{user:s.user,role:s.role,name,email:u?.email||s.user,profession:a?.profession||'',city:a?.city||'',bio:a?.description||'',photo:a?.photo||''})}
  if(url.pathname==='/api/author/profile'&&req.method==='PUT'){const s=adminSession(req);if(!s||s.role!=='author')return json(res,401,{error:'Köşe yazarı girişi gerekli'});const u=db.users.find(x=>String(x.email||x.username||'').toLowerCase()===String(s.user||'').toLowerCase()),email=String(u?.email||s.user||'').toLowerCase(),name=u?.name||s.user,b=await body(req);let a=(db.authors||[]).find(x=>String(x.email||'').toLowerCase()===email);if(!a){a={id:`authors-${Date.now()}-${randomBytes(3).toString('hex')}`,name,email,status:'Aktif',createdAt:new Date().toISOString()};db.authors.unshift(a)}a.profession=String(b.profession||'').trim();a.city=String(b.city||'').trim();a.description=String(b.bio||'').trim();if(b.photo!==undefined)a.photo=String(b.photo||'').trim();a.updatedAt=new Date().toISOString();audit('Yazar profilini güncelledi','authors',a);await save(db);return json(res,200,{profession:a.profession,city:a.city,bio:a.description,photo:a.photo||''})}
  ...
  const parts=url.pathname.split('/').filter(Boolean),name=parts[1],id=parts[2];if(parts[0]!=='api'||!collections.includes(name))return json(res,404,{error:'Bulunamadı'});const current=adminSession(req),role=current.role||'admin';auditUser=current.user||USER;if(!roleAccess[role]?.has(name))return json(res,403,{error:'Bu modül için yetkiniz yok'});const list=db[name],isAuthorUser=x=>(x.systemRole||normalizedRole(x.role))==='author',safe=x=>['accounts','users'].includes(name)?Object.fromEntries(Object.entries(x||{}).filter(([k])=>!['passwordHash','salt','initialPassword'].includes(k))):x;
  if(req.method==='GET'){const rows=name==='users'&&role==='moderator'?list.filter(isAuthorUser):name==='members'?list.filter(x=>approvedStatus(x.status)):list;return json(res,200,id?safe(list.find(x=>x.id===id)||{}):rows.map(safe))}
  if(req.method==='POST'){const b=await body(req);if(name==='users'&&role==='moderator')b.role='Köşe Yazarı';const item={...b,id:`${name}-${Date.now()}-${randomBytes(3).toString('hex')}`,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};let newPlainPassword=null;if(name==='users'){const plain=String(b.password||'').trim()||tempPassword(),salt=randomBytes(16).toString('hex');item.email=String(item.email||item.username||'').trim().toLowerCase();item.username=item.email;item.passwordHash=scryptSync(plain,salt,32).toString('hex');item.salt=salt;item.systemRole=normalizedRole(b.role);item.role=roleLabel(item.systemRole);delete item.password;newPlainPassword=plain;if(item.systemRole==='author'&&!db.authors.some(a=>String(a.email||'').toLowerCase()===item.email)){db.authors.unshift({id:`authors-${Date.now()}-${randomBytes(3).toString('hex')}`,name:item.name||item.email,email:item.email,status:'Aktif',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()})}}else if(name==='members'){const email=String(item.email||'').trim().toLowerCase();item.email=email;if(email&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){let account=db.accounts.find(x=>x.email===email);if(!account){const plain=tempPassword(),salt=randomBytes(16).toString('hex');account={id:`account-${Date.now()}-${randomBytes(3).toString('hex')}`,name:item.name||'',email,phone:item.phone||'',city:item.city||'',profession:item.profession||'',membershipType:item.group||'',group:item.group||'',passwordHash:scryptSync(plain,salt,32).toString('hex'),salt,role:'Site Üyesi',membershipStatus:item.status||'Aktif Üye',createdAt:new Date().toISOString()};db.accounts.push(account);newPlainPassword=plain}item.accountId=account.id}}list.unshift(item);audit('Eklendi',name,item);await save(db);return json(res,201,newPlainPassword?{...safe(item),tempPassword:newPlainPassword}:safe(item))}
  const index=list.findIndex(x=>x.id===id);if(index<0)return json(res,404,{error:'Kayıt bulunamadı'});
  if(req.method==='PUT'){const b=await body(req);if(name==='users'&&role==='moderator'&&!isAuthorUser(list[index]))return json(res,403,{error:'Moderatör yalnızca köşe yazarı hesaplarını yönetebilir'});let resetPlainPassword=null;if(name==='users'){if(role==='moderator')b.role='Köşe Yazarı';if(b.role){b.systemRole=normalizedRole(b.role);b.role=roleLabel(b.systemRole)}if(b.email||b.username){b.email=String(b.email||b.username||'').trim().toLowerCase();b.username=b.email}if(b.password||b.autoPassword){const plain=String(b.password||'').trim()||tempPassword(),salt=randomBytes(16).toString('hex');b.passwordHash=scryptSync(plain,salt,32).toString('hex');b.salt=salt;delete b.password;delete b.autoPassword;resetPlainPassword=plain}}else if(name==='members'){const currentItem=list[index];const email=String(b.email!==undefined?b.email:currentItem.email||'').trim().toLowerCase();if(b.email!==undefined)b.email=email;if(b.autoPassword==='Evet'&&email&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){const plain=tempPassword(),salt=randomBytes(16).toString('hex');let account=db.accounts.find(x=>x.id===currentItem.accountId)||db.accounts.find(x=>x.email===email);if(!account){account={id:`account-${Date.now()}-${randomBytes(3).toString('hex')}`,name:b.name||currentItem.name||'',email,role:'Site Üyesi',membershipStatus:b.status||currentItem.status||'Aktif Üye',createdAt:new Date().toISOString()};db.accounts.push(account)}account.email=email;account.passwordHash=scryptSync(plain,salt,32).toString('hex');account.salt=salt;account.updatedAt=new Date().toISOString();if(b.name)account.name=b.name;if(b.phone)account.phone=b.phone;if(b.city)account.city=b.city;if(b.profession)account.profession=b.profession;if(b.group){account.membershipType=b.group;account.group=b.group}b.accountId=account.id;resetPlainPassword=plain}delete b.autoPassword}list[index]={...list[index],...b,id,updatedAt:new Date().toISOString()};if(name==='applications')syncApplicationApproval(list[index]);if(name==='users'&&list[index].systemRole==='author'&&!db.authors.some(a=>String(a.email||'').toLowerCase()===String(list[index].email||'').toLowerCase()))db.authors.unshift({id:`authors-${Date.now()}-${randomBytes(3).toString('hex')}`,name:list[index].name||list[index].email,email:list[index].email,status:'Aktif',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});audit('Güncellendi',name,list[index]);await save(db);return json(res,200,resetPlainPassword?{...safe(list[index]),tempPassword:resetPlainPassword}:safe(list[index]))}

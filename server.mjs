...
const roleAccess={admin:new Set(collections),sayman:new Set(['dues','duePeriods','payments','businessLedger','bankAccounts','members','memberGroups','settings']),moderator:new Set(['content','events','publications','webinars','galleries','videos','articles','authors','sliders','jobPosts','firms','users','applications','members','invitations','memberMessages']),author:new Set([])};
...
const richTags=new Set(['p','br','strong','b','em','i','u','h2','h3','ul','ol','li','blockquote','a']);
function sanitizeRichHtml(value){let html=String(value||'').trim().replace(/<!--([\s\S]*?)-->/g,'').replace(/<(script|style|iframe|object|embed|form)[^>]*>[\s\S]*?<\/\1>/gi,'');return html.replace(/<\/?([a-z][a-z0-9]*)([^>]*)>/gi,(whole,rawTag,attrs)=>{const tag=String(rawTag).toLowerCase(),closing=whole.startsWith('</');if(!richTags.has(tag))return'';if(closing)return tag==='br'?'':`</${tag}>`;if(tag==='br')return'<br>';if(tag==='a'){const match=String(attrs||'').match(/\bhref\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/i),href=match?.[1]||match?.[2]||match?.[3]||'';if(!/^(https?:\/\/|mailto:|\/|#)/i.test(href))return'<a>';const safe=href.replace(/&/g,'&amp;').replace(/"/g,'&quot;');return`<a href="${safe}" target="_blank" rel="noopener">`}return`<${tag}>`})}
